/**
 * Liopleurodon Library - Misc projects, modules, and R&D in various languages. Copyright (c) Michael Hillman
 * (thisishillman.co.uk)
 * 
* This file is part of the larger, Algorithms project. The Algorithms project is free software: you can redistribute it
 * and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation, either
 * version 3 of the License, or (at your option) any later version. This project is distributed in the hope that it will be
 * useful for educational purposes, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS
 * FOR A PARTICULAR PURPOSE. See the GNU General Public License for more details.
 * 
* You should have received a copy of the GNU General Public License along with the Algorithms project. If not, see the gnu
 * website.
 */
package uk.co.thisishillman.subdivision.root_three;
...
public class RootThree implements SubdivisionAlgorithm, Runnable {
    ...
    /**
     * Sets the Polyhedron object for subdivision, creates a RootThreeUtilities object to handle additional mathematical
     * calculations, finally creates a Thread object & starts the subdivision proceedure,
     *
     ...
     *
     * 1. For every face in the original mesh, we insert a midpoint (defined as the average of the face's three original
     * points). 2. For each vertex in that face, calculate B (a function of the vertex's valence) as (where n is the
     * valence): B = (4 - 2cos(2PI/n)) / 9n 3. Each original vertex in that face in then relaxed following the equation below
     * where P is the original vertex, B is a function of P's valence (see step 2), n is P's valence, S is the resulting
     * relaxed vertex, and M total of all the vertices that ring vertex S. S = (1 - nB)P + BM 4. Next we create three new
     * faces for each face, from the relaxed vertex based on each edge's start vertex -> midpoint -> the relaxed vertex based
     * on that edge's end vertex. 5. Finally the old edges are flipped to connect pairs of midpoints. 6. All faces (carefully
     * ignoring duplicates) are then added to a new polyhedron & pushed to the DrawingFrame
     *
    ...
    /**
     * Given an original vertex as input, this methods grabs the sum of it's neighbouring vertices, applies the B scalar &
     * returns the now relaxed vertex.
     *
    ...
    /**
     * Given an original vertex as input, this simply sums the values of it's neighbouring vertices into one vertex.
     *
    ...
    public static void main(String[] args) {
        ...
        DrawingFrame frame = new DrawingFrame("Root-Three Subdivision", GL.GL_LINES);
